...
app.post("/adduser/:name/:email/:tel/:password/:pid", async (req, res) => {
  ...
  let info = await transporter.sendMail({
    ...
    html: '<h2>click link to verify your email address</h2><br><a href="https://umbrellashareserver.herokuapp.com/verify_email/'+name+'/'+email+'/'+tel+'/'+password+'/'+p_id+'">verify</a>' // html body
  });
...
app.post("/addplace/:latitude/:longitude/:place/:node_ip", async (req, res) => {
  ...
  var addplace = await new Place({latitude:latitude,longitude:longitude,place:place,node_ip:node_ip}).save()
...
app.get('/delete/:node_ip', async (req, res) => {
  ...
  if(realtime){
    console.log(realtime);
    await Rotation.findAndDelete(realtime);
    res.send("blank");
  }
  if(!realtime){
    res.send("error");
